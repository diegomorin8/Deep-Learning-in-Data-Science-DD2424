...
# Load the book_data and get the main characteristics from the book_data
class LoadBook:
    # Constructor
        def __init__(self, book_data_fname  = 'data/goblet.txt'):
            
            # Load the book_data
            self.book_data = open(book_data_fname,'r').read()

            # Get the unique characters as a list
            self.bchars = ''.join(set(self.book_data))
            self.bchars = list(sorted(self.bchars))

            # Dimensions
            self.K = len(self.bchars)
            self.bsize = len(self.book_data)

            # Mappings from char to ind and vice versa, mapped in a dictionary
            self.char_to_ind = { character:index for index, character in enumerate(self.bchars)}
            self.ind_to_char = { index:character for index,character in enumerate(self.bchars)}
...
class RNN:
    ...
    def __init__(self, hidden_dim, learning_rate, input_size, output_size):
        ...
        self.W = np.ones((hidden_dim, hidden_dim))*0.1
        ...
        self.U = np.ones((hidden_dim, self.input_size))*0.1
        ...
        self.V = np.ones((self.output_size, hidden_dim))*0.1
    ...
    def train(self, x, y, i):
        # Forward pass
        loss, p, h = self.forward(x, y,i)
        
        # Backward pass
        dW, dU, dV, db, dc, h = self.backward(x, y, p, h,i)
...
def main():
    data = LoadBook('data/goblet.txt')
    input_size = len(data.bchars)
    output_size = len(data.bchars)
    rnn = RNN(100, 0.1, input_size, output_size)

    seq_length = 25
    losses = []
    smooth_loss = -np.log(1.0/len(data.bchars))*seq_length
    ...
    for e in xrange(n_epochs):
        for i in range(data.bsize / seq_length):
            # Input
            x = [data.char_to_ind[c] for c in data.book_data[i * seq_length:(i + 1) * seq_length]]
            # Output that should be predicted, i.e. next character
            y = [data.char_to_ind[c] for c in data.book_data[i * seq_length + 1:(i + 1) * seq_length + 1]]
	    
            if i % 10000 == 0:
                synth_sample = rnn.synthesize(x[0], 200)
                txt = ''.join([data.ind_to_char[n] for n in synth_sample])
                print txt

            loss = rnn.train(x, y, i)
...
def test():
    ...
    # Input
    x = [data.char_to_ind[c] for c in data.book_data[0:seq_length]]
    # Output that should be predicted, i.e. next character
    y = [data.char_to_ind[c] for c in data.book_data[1:seq_length + 1]]
  
    synth_sample = rnn.synthesize(x[0], 20)
    txt = ''.join([data.ind_to_char[n] for n in synth_sample])
    ...
    print h
    # Backward pass
    dW, dU, dV, db, dc, h = rnn.backward(x, y, p, h)
    
    rnn.grad_check([rnn.W, rnn.U, rnn.V, rnn.b, rnn.c], [dW, dU, dV, db, dc], x, y)
