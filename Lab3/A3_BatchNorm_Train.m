...
[W_end2, b_end2, val_loss2, train_loss2] = MiniBatchGD2(X_batch, Y_batch, X_val, Y_val, GDparams, W, b, lambda, 2, eta_decay, mW, mb, rho, X_test, y_test);
...
acc2 = ComputeAccuracy(X_test, y_test, W_end2, b_end2);
...
hold on
plot(train_loss2);
legend(sprintf('Validation data Acc = %g', acc),sprintf('Training data Acc = %g', acc), sprintf('Validation data Acc = %g', acc2),sprintf('Training data Acc = %g', acc2) )
title(sprintf(' Validation - Trining loss - eta = %g - rho = %g - decay = %g - Num layers = %g', GDparams.eta, 0.5, 1, 3))


%% TRY TO TRAIN WITH 3 LAYERS
...
title(sprintf(' Validation - Trining loss - eta = %g - rho = %g - decay = %g - Num layers = %g', GDparams.eta, 0.9, 0.95, 3))

%% 3 LAYER NO NORMALIZATION - lower eta
%
% The parameters that we can set are chosen accordingly to the lecture
% notes
GDparams.n_batch = 100;
GDparams.n_epochs = 10;
GDparams.eta = 0.0319*0.5;
lambda = 0.0043;
eta_decay = 0.95;
rho = 0.9;
%% Check if with 2 layers we get the same result
...
%% 3 LAYER - NO NORMALIZATION higher eta
%
% The parameters that we can set are chosen accordingly to the lecture
% notes
GDparams.n_batch = 100;
GDparams.n_epochs = 10;
GDparams.eta = 0.0319*5;
lambda = 0.0043;
eta_decay = 0.95;
rho = 0.9;
%% Check if with 2 layers we get the same result
